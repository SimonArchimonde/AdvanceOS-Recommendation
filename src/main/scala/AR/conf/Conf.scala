...
case class Conf() {
  ...
  var appName: String = "Question1"

  var numPartitionAB: Int = 336 // partition number used in AB stage
  var numPartitionCD: Int = 168 //partition number used in CD stage
  ...
  def getConf(param: Array[String]): Conf = {
    ...
    (0 to param.length / 2 - 1).foreach { i =>
      param(2 * i) match {
        ...
        case "--tempFilePath"   => conf.tempFilePath = param(2 * i + 1)
        ...
        case "--spark_memory_fraction" =>
          conf.spark_memory_fraction = param(2 * i + 1)
        case "--spark_memory_storage_Fraction" =>
          conf.spark_memory_storage_Fraction = param(2 * i + 1)
        case "--spark_shuffle_spill_compress" =>
          conf.spark_shuffle_spill_compress = param(2 * i + 1)
        case "--spark_memory_offHeap_enable" =>
          conf.spark_memory_offHeap_enable = param(2 * i + 1)
        case "--spark_memory_offHeap_size" =>
          conf.spark_memory_offHeap_size = param(2 * i + 1)
        case "--spark_executor_cores_AB" =>
          conf.spark_executor_cores_AB = param(2 * i + 1)
        case "--spark_cores_max_AB" =>
          conf.spark_cores_max_AB = param(2 * i + 1)
        case "--spark_executor_cores_CD" =>
          conf.spark_executor_cores_CD = param(2 * i + 1)
        case "--spark_cores_max_CD" =>
          conf.spark_cores_max_CD = param(2 * i + 1)
        case "--spark_executor_instances" =>
          conf.spark_executor_instances = param(2 * i + 1)
        case "--spark_driver_cores" =>
          conf.spark_driver_cores = param(2 * i + 1)
        case "--spark_driver_memory" =>
          conf.spark_driver_memory = param(2 * i + 1)
        case "--spark_executor_memory_AB" =>
          conf.spark_executor_memory_AB = param(2 * i + 1)
        case "--spark_executor_memory_CD" =>
          conf.spark_executor_memory_CD = param(2 * i + 1)
        case "--spark_default_parallelism" =>
          conf.spark_parallelism_CD = param(2 * i + 1)
        case _ => {
    ...
    assert(
      flag - 2 == 0,
      "Maybe you forget to set the inputFilePath or outputFilePath"
    )
    conf
  }
