...
object RecPartUserRdd {
  class RuleToCompare(confidence_i: Double, rec_i: String)
      extends Comparable[RuleToCompare] {
  ...
  class MyOrdering2[T] extends Ordering[T] {
    def compare(x: T, y: T): Int = {
      (x, y) match {
        case (null, null) => 0
        case (iX: (HashSet[Int], Long), iY: (HashSet[Int], Long)) =>
          iX._1.size compare iY._1.size
      }
    }
  }
  ...
  def run(myConf: Conf, conf: SparkConf): Unit = {
    ...
    val associationRulesRdd = sc
      .objectFile[RuleNewDef](myConf.tempFilePath)
      .sortBy(
        rule => new RuleToCompare(rule.confidence, rule.consequent(0).toString),
        ascending = false,
        partitionNum
      )
    ...
    //fragmentation
    val userDataRdd = sc
      .textFile(myConf.inputFilePath + "/U.dat", partitionNum)
      .map(items => items.trim.split(' ').map(s => s.toInt).sorted)
      .map(items => HashSet(items: _*))
      .zipWithIndex()
    //      .cache()
      ...
      while (iter.hasNext) {
        ...
        loop.breakable {
          for (i <- associationRules.value.indices) {
            ...
            //判断规则是否符合条件
            if (
              user._1.size >= associationRules
                .value(i)
                .antecedent
                .length && !user._1.contains(end)
            ) {
    ...
    val userRdd_25 = userDataRdd
      .filter(item => item._1.size <= 25)
      .repartition(partitionNum)(new MyOrdering2[(HashSet[Int], Long)])
      .mapPartitions(calFunc)
      .cache()

    val userRdd_30 = userDataRdd
      .filter(item => item._1.size > 25 && item._1.size <= 50)
      .repartition(partitionNum)(new MyOrdering2[(HashSet[Int], Long)])
      .mapPartitions(calFunc)
      .cache()

    val userRdd_60 = userDataRdd
      .filter(item => item._1.size > 50 && item._1.size <= 100)
      .repartition(partitionNum)(new MyOrdering2[(HashSet[Int], Long)])
      .mapPartitions(calFunc)
      .cache()

    val userRdd_100 = userDataRdd
      .filter(item => item._1.size > 100 && item._1.size <= 500)
      .repartition(partitionNum)(new MyOrdering2[(HashSet[Int], Long)])
      .mapPartitions(calFunc)
      .cache()

    val userRdd_1195 = userDataRdd
      .filter(item => item._1.size > 500)
      .repartition(partitionNum)(new MyOrdering2[(HashSet[Int], Long)])
      .mapPartitions(calFunc)
      .cache()

    (userRdd_25 ++ userRdd_30 ++ userRdd_60 ++ userRdd_100 ++ userRdd_1195)
      .sortByKey()
      .map(userRec => {
        userRec._2
      })
